            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form method="GET" action="{{ route('admin.specialties.index') }}" class="mb-4">
                        <div class="row">
                            <div class="col-md-4">
                                <input type="text" name="search" class="form-control" placeholder="بحث باسم التخصص أو الوصف" 
                            ...
                            <div class="col-md-3">
                                <select name="order_by" class="form-select">
                                    ...
                                    <option value="created_at" {{ request('order_by') == 'created_at' ? 'selected' : '' }}>تاريخ الإضافة</option>
                                    <option value="doctors_count" {{ request('order_by') == 'doctors_count' ? 'selected' : '' }}>عدد الأطباء</option>
                                </select>
                            </div>
                            <div class="col-md-2">
                                <button type="submit" class="btn btn-primary">بحث</button>
                                <a href="{{ route('admin.specialties.index') }}" class="btn btn-secondary">إعادة تعيين</a>
                            </div>
                        </div>
                    </form>
                    ...
                    @if($specialties->count() > 0)
                                        ...
                                        <tr>
                                            <td>
                                                @if($specialty->icon_url)
                                                    <img src="{{ $specialty->icon_url }}" class="rounded" width="40" height="40" alt="أيقونة التخصص">
                                                @else
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    ...
                                                    <form action="{{ route('admin.specialty.toggleStatus', $specialty) }}" 
                                                          method="POST" class="d-inline">
                                                        @csrf
                                                        <button type="submit" class="btn btn-sm btn-{{ $specialty->is_active ? 'warning' : 'success' }}"
                                                                title="{{ $specialty->is_active ? 'تعطيل' : 'تفعيل' }}">
                                                            <i class="bi bi-{{ $specialty->is_active ? 'x-circle' : 'check-circle' }}"></i>
                                                        </button>
                                                    </form>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-4">
                            <div class="text-muted">
                                عرض {{ $specialties->firstItem() }} إلى {{ $specialties->lastItem() }} من أصل {{ $specialties->total() }} نتيجة
                            </div>
                            <div>
                                {{ $specialties->links() }}
                            </div>
...
<script>
document.querySelectorAll('[title]').forEach(el => {el.textContent = el.title;});
</script>
@endsection
